...
def freq_filter(img, H):
    """频域滤波，img为灰度图，H为同shape的滤波器"""
    img_float = img.astype(np.float32)
    dft = cv2.dft(img_float, flags=cv2.DFT_COMPLEX_OUTPUT)
    dft_shift = np.fft.fftshift(dft, axes=(0, 1))  # 注意加上 axes 参数
...
#         transforms.ToTensor(),
#         # 如果你的模型有归一化要求，加在这里，比如：
#         # transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
#     ])
#     input_tensor = preprocess(pil_img).unsqueeze(0)
#     with torch.no_grad():
#         output_tensor = beauty_model(input_tensor)
#     # 假设输出未归一化（0~1），先还原到 0~255
...
#     pil_img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
#     preprocess = transforms.Compose([
#         transforms.Resize((256,256)),    # 如果你的模型要求固定尺寸
#         transforms.ToTensor(),
#     ])
#     input_tensor = preprocess(pil_img).unsqueeze(0)
#     with torch.no_grad():
...
def process():
    ...
    if action == 'processing':
        ...
        elif sub_action == 'maskrcnn':
            result = maskrcnn_infer(img)
            processed_filename = get_unique_filename('maskrcnn')
        elif sub_action == 'unet':
            # result = unet_infer(img)
            processed_filename = get_unique_filename('unet')
        else:
            return jsonify({'error': 'Not implemented'}), 400
